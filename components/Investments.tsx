...
const Investments: React.FC = () => {
  ...
  return (
    <div className="space-y-8 sm:space-y-10 animate-in slide-in-from-bottom-6 duration-700 max-w-screen-xl mx-auto">
      {/* Portfolio Summary Header */}
      <section className="bg-gradient-to-br from-[#0a0f1d] to-[#1a237e] rounded-[2.5rem] sm:rounded-[3rem] p-6 sm:p-8 text-white relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#2979FF]/10 rounded-full -mr-32 -mt-32 blur-[100px]"></div>
        <div className="relative z-10 space-y-6">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">Patrimônio Investido</p>
              <h2 className="text-3xl sm:text-4xl font-black tracking-tighter">
                R$ {totalInvested.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </h2>
            </div>
            <div className="w-12 h-12 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/10 shrink-0">
      ...
      <header className="space-y-6">
        <div className="flex justify-between items-end px-2">
           <div>
              ...
              <h3 className="text-xl sm:text-2xl font-black text-[#0a0f1d] tracking-tighter uppercase">Oportunidades</h3>
           </div>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-6 px-6 py-2">
           {['All', 'Renda Fixa', 'Tesouro', 'Fundos', 'Ações'].map(cat => (
             <button
               ...
               className={`px-5 py-3 rounded-full text-[9px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${
                 filterCategory === cat ? 'bg-[#0a0f1d] text-white border-[#0a0f1d] shadow-lg shadow-slate-200' : 'bg-white text-slate-400 border-slate-50'
               }`}
             >
      ...
      {/* Investment Product Cards Grid - Responsivo 1 col mobile / 2 col desktop */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        {filtered.map((inv) => (
          <div 
            key={inv.id} 
            onClick={() => setSelectedInvestment(inv)}
            className="group bg-white border border-slate-100 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-6 transition-all hover:border-[#2979FF]/30 hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)] active:scale-[0.98] cursor-pointer relative overflow-hidden flex flex-col justify-between"
          >
            <div className="space-y-5">
              <div className="flex justify-between items-start">
                 <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-[#0a0f1d] group-hover:bg-[#0a0f1d] group-hover:text-white transition-all duration-500">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                 ...
                 <div className="text-right">
                    ...
                    <p className="text-[10px] font-black text-slate-600">{inv.liquidity}</p>
                 </div>
              </div>
            </div>
      ...
      {selectedInvestment && (
        <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center">
          ...
          <div className="bg-white w-full sm:max-w-lg rounded-t-[3rem] sm:rounded-[3rem] shadow-2xl z-10 animate-in slide-in-from-bottom-full duration-500 max-h-[90dvh] overflow-y-auto pb-12">
             ...
             <div className="p-8 sm:p-10 space-y-8">
                <header className="space-y-6">
                   <div className="flex justify-between items-start">
                      <div className="space-y-1">
                         <div className="flex items-center gap-2">
                            <Mountain size={14} className="text-[#2979FF]" />
                            <span className="text-[8px] font-black text-[#2979FF] uppercase tracking-[0.4em]">Exclusivo Montbank</span>
                         </div>
                         <h3 className="text-2xl sm:text-3xl font-black text-[#0a0f1d] leading-none tracking-tighter uppercase">{selectedInvestment.name}</h3>
                      </div>
                      <button 
                        onClick={() => setSelectedInvestment(null)}
                        className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 hover:text-red-500 transition-colors"
                      >
                         <X size={20} />
                      </button>
                   </div>
                   ...
                   <div className="grid grid-cols-3 gap-2">
                      <div className="bg-slate-50 p-3 rounded-2xl text-center">
                         ...
                         <p className="text-[11px] font-black text-[#0a0f1d]">{selectedInvestment.yield}</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-2xl text-center">
                         <p className="text-[7px] font-black text-slate-300 uppercase tracking-widest mb-1">Mínimo</p>
                         <p className="text-[11px] font-black text-[#0a0f1d]">R$ {selectedInvestment.minInvestment}</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-2xl text-center">
                         <p className="text-[7px] font-black text-slate-300 uppercase tracking-widest mb-1">Prazo</p>
                         <p className="text-[11px] font-black text-[#0a0f1d]">{selectedInvestment.liquidity}</p>
                      </div>
                   </div>
                </header>
                ...
                <section className="space-y-6">
                   <div className="bg-slate-50 p-8 rounded-[2.5rem] text-center border border-slate-100">
                      <p className="text-[9px] font-black text-[#2979FF] uppercase tracking-[0.4em] mb-4">Investimento Inicial</p>
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <input 
                          ...
                          placeholder="0,00"
                          className="bg-transparent text-4xl sm:text-5xl font-black text-[#0a0f1d] w-full focus:outline-none text-center placeholder:text-slate-100 tracking-tighter"
                        />
                      </div>
                   </div>

                   <button 
                     onClick={() => {alert('Ordem de investimento enviada.'); setSelectedInvestment(null);}}
                     className="w-full bg-[#0a0f1d] text-white py-6 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.5em] shadow-2xl active:scale-95 transition-transform flex items-center justify-center gap-3"
                   >
